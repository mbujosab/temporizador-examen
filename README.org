...
1. *Crea la rama =gh-pages=*:
   ...
   | D         | Mates2-90.html             |          |            |          |        |
   | M         | README.org                 |          |            |          |        |
   | D         | Temporizador-105.html      |          |            |          |        |
   | D         | Temporizador-120.html      |          |            |          |        |
   | D         | Temporizador-60.html       |          |            |          |        |
   | D         | Temporizador-90.html       |          |            |          |        |
   | D         | Temporizador-demo.html     |          |            |          |        |
   | M         | examtimer.org              |          |            |          |        |
   | D         | index.html                 |          |            |          |        |
   | M         | index.org                  |          |            |          |        |
   | rm        | '.gitignore'               |          |            |          |        |
   ...
   | rm        | 'Mates2-90.html'           |          |            |          |        |
   | rm        | 'README.org'               |          |            |          |        |
   | rm        | 'Temporizador-105.html'    |          |            |          |        |
   | rm        | 'Temporizador-120.html'    |          |            |          |        |
   | rm        | 'Temporizador-60.html'     |          |            |          |        |
   | rm        | 'Temporizador-90.html'     |          |            |          |        |
   | rm        | 'Temporizador-demo.html'   |          |            |          |        |
   | rm        | 'css/bigblow_inline.theme' |          |            |          |        |
   | rm        | 'examtimer.org'            |          |            |          |        |
   | rm        | 'index.html'               |          |            |          |        |
   | rm        | 'index.org'                |          |            |          |        |
   | rm        | 'photo.jpg'                |          |            |          |        |
   | [gh-pages | (commit-raíz)              | 93fa452] | Initialize | gh-pages | branch |

2. *Configurar GitHub Pages*:
   - Ir a la configuración del repositorio en GitHub (Settings).
   - Buscar la sección "Pages".
   - En "Source", seleccionar la rama =gh-pages= y el directorio =/(root)= o =/docs= según tu preferencia (en este repositorio voy a usar =/docs=).
   - Guarda los cambios.
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Set up Emacs
        uses: emacs-actions/setup-emacs@v2
