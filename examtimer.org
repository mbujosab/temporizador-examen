#+TITLE: Código fuente de los temporizadores de examen

Este archivo contiene el código fuente literate programming (orgmode) que genera
los temporizadores HTML mediante noweb tangling. Los temporizadores se organizan
por tipo (genérico, por asignatura) y duración.

* Genérico

** 15 minutos

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-15.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 15 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 20 minutos

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-20.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 20 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 30 minutos

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-30.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 30 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 45 minutos

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-45.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 45 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 1 hora

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-60.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 60 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 1 hora y cuarto

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-75.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 75 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 1 hora y media

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-90.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 90 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 1 hora y tres cuartos

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-105.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 105 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 1 hora y 50 minutos

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-110.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 110 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 2 horas

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-120.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 120 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 2 horas y cuarto

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-135.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 135 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

** 2 horas y media

#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-150.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 150 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

* Asignaturas

** Matemáticas II

*** Matemáticas II de 1 hora y media

#+BEGIN_SRC html :tangle docs/temporizadores/Mates2-90.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Exam timer</title>

    <<DURACIÓN 90 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN MATEMÁTICAS II>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

*** Matemáticas II de 1 hora y tres cuartos

#+BEGIN_SRC html  :tangle docs/temporizadores/Mates2-105.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Exam timer</title>

    <<DURACIÓN 105 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN MATEMÁTICAS II>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

*** Matemáticas II de 2 horas

#+BEGIN_SRC html  :tangle docs/temporizadores/Mates2-120.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Exam timer</title>

    <<DURACIÓN 120 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN MATEMÁTICAS II>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC


** Econometría aplicada

*** Econometría aplicada de 1 hora y media

#+BEGIN_SRC html :tangle docs/temporizadores/EctrAp-90.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Exam timer</title>

    <<DURACIÓN 90 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN ECONOMETRÍA APLICADA>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

*** Econometría aplicada de 1 hora y tres cuartos

#+BEGIN_SRC html  :tangle docs/temporizadores/EctrAp-105.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Exam timer</title>

    <<DURACIÓN 105 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN ECONOMETRÍA APLICADA>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC

*** Econometría aplicada de 2 horas

#+BEGIN_SRC html  :tangle docs/temporizadores/EctrAp-120.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Exam timer</title>

    <<DURACIÓN 120 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN ECONOMETRÍA APLICADA>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC



* Partes específicas de código

** Instrucciones específicas de cada asignatura

*** General

#+name: INFORMACIÓN GENERAL
#+BEGIN_SRC html
  <section class="infos">
      <span class="before">Bienvenido a este examen</span>
      <header>
          <h1>Instrucciones importantes</h1>
      </header>
      <ul>
        <li class="before during">Ponga su <strong>documento de identificación</strong> sobre la mesa.</li>
        <li class="before during"><strong>Retire cualquier dispositivo electrónico</strong> (móviles, smartwatches, etc.). La posibilidad de acceso a alguno se considerará un intento de copia. No use gafas oscuras y mantenga los oídos visibles.</li>
        <li class="before during">Los intentos de copia son faltas graves: suponen un "cero" en actas y posibles sanciones académicas.</li>
        <li class="before during" ><strong>Lea las instrucciones</strong> del examen detenidamente.</li>
        <li class="before during homestretch"><strong>Ponga su nombre</strong> en cada hoja.</strong></li>
        <li class="before during homestretch"><strong>Indique el grupo</strong> en el que está matriculado.</strong></li>
        <li class="during homestretch">Al finalizar, entrege todas las hojas juntas.</strong></li>
        <li class="before">Le deseamos mucho éxito.</li>
        <li class="during">Si <strong>necesita más papel</strong>, levante la mano y el personal encargado le atenderá.</li>
        <li class="homestretch">Si desea entregar el examen y abandonar el aula, hágalo discretamente para no distraer a sus compañeros.</li>
        <li class="after">El examen ha concluido.</li>
<li class="after">Espere en su asiento a que el profesor recoja su examen. Entréguelo en cuanto se lo pidan. No demore la recogida.</li>
        <li class="after">Gracias por su participación.</li>
        <li class="after">Le deseamos un buen descanso y éxito en sus demás actividades académicas.</li>
      </ul>
  </section>
#+END_SRC


*** Matemáticas

#+name: INFORMACIÓN MATEMÁTICAS II
#+BEGIN_SRC html
  <section class="infos">
      <span class="before">Examen de</span>
      <header>
          <h1>Matemáticas II</h1>
          <h2>(Grado en Economía)</h2>
      </header>
      <ul>
        <li class="before during">Ponga su <strong>documento de identificación</strong> sobre la mesa.</li>
        <li class="before during"><strong>Retire cualquier dispositivo electrónico</strong> (móviles, smartwatches, calculadoras, etc.).  La posibilidad de acceso a alguno se considerará un intento de copia. No use gafas oscuras y mantenga los oídos visibles.</li>
        <li class="before during">Los intentos de copia son faltas graves: suponen un "cero" en actas y posibles sanciones académicas.</li>
        <li class="before during" ><strong>Lea las instrucciones</strong> del examen detenidamente.</li>
        <li class="before during homestretch"><strong>Ponga su nombre</strong> en cada hoja.</strong></li>
        <li class="before during homestretch"><strong>Indique el grupo</strong> en el que está matriculado.</strong></li>
        <li class="before during homestretch">Al finalizar, entrege todas las hojas juntas (dentro de uno de los pliegos).</strong></li>
        <li class="before">Les deseamos mucho éxito.</li>
        <li class="during">Si <strong>necesita más hojas</strong>, levante la mano. El personal encargado le proporcionará las que necesite.</li>
        <li class="homestretch">Si desea entregar el examen y abandonar el aula, hágalo discretamente para no distraer a sus compañeros.</li>
        <li class="after">El examen ha concluido.</li>
        <li class="after">Espere en su asiento a que el profesor recoja su examen. Entréguelo en cuanto se lo pidan. No demore la recogida.</li>
        <li class="after">Gracias por su participación.</li>
        <li class="after">Le deseamos un buen descanso y éxito en sus demás actividades académicas.</li>
      </ul>
  </section>
#+END_SRC



*** Econometría aplicada

#+name: INFORMACIÓN ECONOMETRÍA APLICADA
#+BEGIN_SRC html
  <section class="infos">
      <span class="before">Examen de</span>
      <header>
          <h1>Econometría Aplicada</h1>
      </header>
      <ul>
        <li class="before during">Ponga su <strong>documento de identificación</strong> sobre la mesa.</li>
        <li class="before during"><strong>Retire cualquier dispositivo electrónico</strong> (móviles, smartwatches, calculadoras, etc.). La posibilidad de acceso a alguno se considerará un intento de copia. No use gafas oscuras y mantenga los oídos visibles.</li>
        <li class="before during">Los intentos de copia son faltas graves: suponen un "cero" en actas y posibles sanciones académicas.</li>
        <li class="before during" ><strong>Lea las instrucciones</strong> del examen detenidamente.</li>
        <li class="before during homestretch"><strong>Ponga su nombre</strong> en cada hoja.</strong></li>
        <li class="before during homestretch"><strong>Indique el grupo</strong> en el que está matriculado.</strong></li>
        <li class="before during homestretch">Al finalizar, entrege todas las hojas juntas (dentro de uno de los pliegos).</strong></li>
        <li class="before">Les deseamos mucho éxito.</li>
        <li class="during">Si <strong>necesita más hojas</strong>, levante la mano. El personal encargado le proporcionará las que necesite.</li>
        <li class="homestretch">Si desea entregar el examen y abandonar el aula, hágalo discretamente para no distraer a sus compañeros.</li>
        <li class="after">El examen ha concluido.</li>
        <li class="after">Espere en su asiento a que el profesor recoja su examen. Entréguelo en cuanto se lo pidan. No demore la recogida.</li>
        <li class="after">Gracias por su participación.</li>
        <li class="after">Le deseamos un buen descanso y éxito en sus demás actividades académicas.</li>
      </ul>
  </section>
#+END_SRC



** Duración de examen


#+name: DURACIÓN 15 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 15;
    var minutosfinales = 5;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC


#+name: DURACIÓN 20 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 20;
    var minutosfinales = 5;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC


#+name: DURACIÓN 30 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 30;
    var minutosfinales = 5;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC


#+name: DURACIÓN 45 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 45;
    var minutosfinales = 15;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC


#+name: DURACIÓN 60 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 60;
    var minutosfinales = 15;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC


#+name: DURACIÓN 75 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 75;
    var minutosfinales = 15;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC


#+name: DURACIÓN 90 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 90;
    var minutosfinales = 15;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC

#+name: DURACIÓN 105 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 105;
    var minutosfinales = 15;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC

#+name: DURACIÓN 110 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 110;
    var minutosfinales = 15;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC

#+name: DURACIÓN 120 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 120;
    var minutosfinales = 15;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC

#+name: DURACIÓN 135 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 135;
    var minutosfinales = 15;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC

#+name: DURACIÓN 150 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 150;
    var minutosfinales = 15;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC

#+name: TEXTO DE LA ESFERA
#+BEGIN_SRC html
var remainingLabel = "m";
var remainingAboveContent = "";
var remainingBelowContent = "TIEMPO RESTANTE";
var startContent = "START";
var endContent = "END";
#+END_SRC


* Partes de código común a todos los temporizadores

Estos bloques de código noweb se reutilizan en todos los temporizadores mediante
la sintaxis <<nombre-bloque>>.

#+name: definición de estilo
#+BEGIN_SRC html
<style>
    * {
        margin: 0;
        padding: 0;
        box-sizing: border-box;
    }
    html, body {
        background-color: black;
        font-family: Helvetica, Arial, sans-serif;
        font-size: 28px;
        color: white;
    }
    body {
        display: flex;
    }
    .infos {
        flex: 0.6;
        padding: 1.5rem;
        height: 100vh;
        overflow-y: scroll;
        background-color: #333;
    }
    header {
        margin-bottom: 0.666em;
    }
    h1 {
        font-size: 1.5em;
    }
    h2 {
        font-size: 1em;
    }
    ul {
        list-style: none;
    }
    li:before {
        content: "–";
        padding-right: 0.5em;
    }
    li {
        padding-left: 1em;
        text-indent: -1em;
        line-height: 1.5em;
        margin-bottom: 0.5em;
    }
    .timer {
        flex: 0.4;
        height: 100vh;
        padding: 1.5rem;
    }
    svg {
        width: 100%;
        height: 100%;
        font-family: Helvetica, Arial, sans-serif;
        letter-spacing: 1px;
    }
</style>
#+END_SRC

#+name: sección temporizador
#+BEGIN_SRC html
<section class="timer">
    <svg id="timer">
        <path id="spinner-background" fill="none" stroke="#555" />
        <path id="spinner" fill="none" stroke="white" />
        <text id="remaining" fill="white" text-anchor="middle" alignment-baseline="middle"></text>
        <text id="remaining-above" fill="white" text-anchor="middle" alignment-baseline="middle"></text>
        <text id="remaining-below" fill="white" text-anchor="middle" alignment-baseline="middle"></text>
        <path id="start-end-circle" fill="none" stroke="none" />
        <text id="start-end" fill="white" text-anchor="middle">
            <textPath id="start-end-path" xlink:href="#start-end-circle"></textPath>
        </text>
    </svg>
</section>
#+END_SRC


El =15= de la línea =if (remaining > 15 * 60 * 1000) {= indica cuántos son los minutos finales del examen en los que se muestra un texto de salida del examen si aún no ha finalizado el tiempo fijado para el examen.
#+name: script funcionamiento
#+BEGIN_SRC html
<script>

    // via https://stackoverflow.com/a/18473154
    function polarToCartesian(centerX, centerY, radius, angleInDegrees) {
        let angleInRadians = (angleInDegrees - 90) * Math.PI / 180.0;

        return {
            x: centerX + (radius * Math.cos(angleInRadians)),
            y: centerY + (radius * Math.sin(angleInRadians))
        };
    }

    // via https://stackoverflow.com/a/18473154
    function describeArc(x, y, radius, startAngle, endAngle) {
        let start = polarToCartesian(x, y, radius, endAngle);
        let end = polarToCartesian(x, y, radius, startAngle);

        let largeArcFlag = endAngle - startAngle <= 180 ? "0" : "1";

        let d = [
            "M", start.x, start.y,
            "A", radius, radius, 0, largeArcFlag, 0, end.x, end.y
        ].join(" ");

        return d;
    }

    // via https://github.com/doersino/markdeep-slides/blob/master/markdeep-slides/markdeep-slides.js
    function toggleFullscreen() {
        var fullscreen = !!(window.fullScreen || document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement);

        if (fullscreen) {
            if (document.exitFullscreen) {
                document.exitFullscreen();
            } else if (document.webkitExitFullscreen) {
                document.webkitExitFullscreen();
            } else if (document.mozCancelFullScreen) {
                document.mozCancelFullScreen();
            } else if (document.msExitFullscreen) {
                document.msExitFullscreen();
            }
        } else {
            var root = document.documentElement;
            if (root.requestFullscreen) {
                root.requestFullscreen();
            } else if (root.mozRequestFullScreen) {
                root.mozRequestFullScreen();
            } else if (root.webkitRequestFullscreen) {
                root.webkitRequestFullscreen();
            } else if (root.msRequestFullscreen) {
                root.msRequestFullscreen();
            }
        }
    }

    var started = false;
    var s;
    var e;

    var paused = false;
    var p;
    var pPrev = 0;
    var pCurr = 0;
    var pTotal = 0;
    var eWithPauses;

    // resume timer after a reload
    if (window.location.hash) {
        let loc = window.location.hash.substring(1);
        let [rawS, rawPPrev] = loc.split("+");

        s = new Date(parseInt(rawS) * 1000);
        pPrev = parseInt(rawPPrev * 1000);
        e = new Date(s.getTime() + duration*60000);

        started = true;
    }

    // handle key presses
    document.body.onkeyup = function(event) {

        // space bar => start timer or pause/unpause
        if (event.keyCode == 32) {
            if (!started) {
                s = new Date();
                e = new Date(s.getTime() + duration*60000);

                started = true;
                window.location.hash = Math.round(s.getTime() / 1000) + "+" + pTotal;
            } else {
                if (!paused) {
                    p = new Date();
                } else {
                    pPrev += pCurr;
                    pCurr = 0;
                }
                paused = !paused;
            }
        }

        // f key => toggle fullscreen
        if (event.keyCode == 70) {
            toggleFullscreen();
        }
    };

    // hide cursor once it hasn't been moved for 2s
    // based on https://stackoverflow.com/a/31798987
    var cursorTimeout;
    document.body.onmousemove = function() {
        if (document.body.style.cursor != "none") {
            if (cursorTimeout) {
                clearTimeout(cursorTimeout);
            }
        } else {
            document.body.style.cursor = "auto";
        }

        cursorTimeout = setTimeout(function() {
            document.body.style.cursor = "none";
        }, 2000);
    };

    // run the following 10 times a second. yes, much of this could be run
    // only once, but none of it is massively resource-intensive and keeping
    // it this way automatically adjusts the size of everything in case the
    // browser window is resized or the scaling factor is changed while the
    // timer is started
    setInterval(function() {
        let w = document.getElementById("timer").clientWidth;
        let h = document.getElementById("timer").clientHeight;
        let size = Math.min(h, w);

        // compute line thicknesses and font sizes
        let spinnerArcThickness = size / 25;
        let spinnerBackgroundArcThickness = size / 60;
        let remainingFontSize = size / 5;
        let remainingAboveFontSize = size / 15;
        let remainingBelowFontSize = size / 15;
        let startEndFontSize = size / 25;

        // define spinner/arc position
        let spinnerRadius = size / 2 - spinnerArcThickness / 2;
        let spinnerX = w / 2;
        let spinnerY = h / 2;

        // define position of labels
        let remainingY = spinnerY;
        let remainingAboveY = remainingY - .7 * (remainingFontSize + remainingAboveFontSize);
        let remainingBelowY = remainingY + .6 * (remainingFontSize + remainingBelowFontSize);
        // weird: the spacing's off if this ^ is the same as the other one

        // shift labels if only one is set
        if (remainingAboveContent && !remainingBelowContent) {
            remainingY += .25 * (remainingFontSize + remainingAboveFontSize);
            remainingAboveY += .25 * (remainingFontSize + remainingAboveFontSize);
        }
        if (!remainingAboveContent && remainingBelowContent) {
            remainingY -= .25 * (remainingFontSize + remainingAboveFontSize);
            remainingBelowY -= .25 * (remainingFontSize + remainingAboveFontSize);
        };

        // compute radius of arc on which start and end times will be placed
        let startEndCircleRadius = spinnerRadius - startEndFontSize;

        // apply computed values
        document.getElementById("spinner").setAttribute("stroke-width", spinnerArcThickness);
        document.getElementById("spinner-background").setAttribute("stroke-width", spinnerBackgroundArcThickness);
        document.getElementById("remaining").setAttribute("font-size", remainingFontSize);
        document.getElementById("remaining").setAttribute("x", spinnerX);
        document.getElementById("remaining").setAttribute("y", remainingY);
        document.getElementById("remaining-above").setAttribute("font-size", remainingAboveFontSize);
        document.getElementById("remaining-above").innerHTML = remainingAboveContent;
        document.getElementById("remaining-above").setAttribute("x", spinnerX);
        document.getElementById("remaining-above").setAttribute("y", remainingAboveY);
        document.getElementById("remaining-below").setAttribute("font-size", remainingBelowFontSize);
        document.getElementById("remaining-below").innerHTML = remainingBelowContent;
        document.getElementById("remaining-below").setAttribute("x", spinnerX);
        document.getElementById("remaining-below").setAttribute("y", remainingBelowY);
        document.getElementById("start-end-circle").setAttribute("d", describeArc(spinnerX, spinnerY, startEndCircleRadius, 0, 359.999));
        document.getElementById("start-end").setAttribute("font-size", startEndFontSize);
        document.getElementById("start-end").setAttribute("x", Math.PI * startEndCircleRadius);

        if (!started) {
            document.getElementById("timer").style.opacity = 0.5;

            // if timer not yet started: show total time
            document.getElementById("spinner").setAttribute("d", describeArc(spinnerX, spinnerY, spinnerRadius, 0, 359.999));
            document.getElementById("spinner-background").setAttribute("d", describeArc(spinnerX, spinnerY, spinnerRadius, 0, 359.999));
            document.getElementById("remaining").innerHTML = duration + remainingLabel;

        } else {

            // handle pauses
            if (paused) {
                pCurr = (new Date()).getTime() - p.getTime();
                history.replaceState({}, '', '#' + Math.round(s.getTime() / 1000) + "+" + Math.round(pTotal / 1000));
            }
            pTotal = pPrev + pCurr;
            eWithPauses = new Date(e.getTime() + pTotal);

            // if timer started: show start and end times and update timer
            document.getElementById("start-end-path").innerHTML = startContent + ": " + s.toTimeString().substring(0,5) + " – " + endContent + ": " + eWithPauses.toTimeString().substring(0,5);

            // tick, tock, tick, tock, ...
            let remaining = Math.max(eWithPauses.getTime() - (new Date()).getTime(), 0);
            let remainingDeg = remaining / (duration * 60 * 1000) * 360;

            if (!paused && remaining > 0) {
                document.getElementById("timer").style.opacity = 1.0;
            } else {
                document.getElementById("timer").style.opacity = 0.5;
            }

            document.getElementById("spinner").setAttribute("d", describeArc(spinnerX, spinnerY, spinnerRadius, 360 - remainingDeg, 360));
            document.getElementById("spinner-background").setAttribute("d", describeArc(spinnerX, spinnerY, spinnerRadius, 0, 359.999));
            document.getElementById("remaining").innerHTML = Math.ceil(remaining / (60 * 1000)) + remainingLabel;
        }

        // show and hide instructions depending on state
        let state;
        if (!started) {
            state = "before";
        } else {
            var remaining = Math.max(eWithPauses.getTime() - (new Date()).getTime(), 0);
            if (remaining > minutosfinales * 60 * 1000) {
                state = "during";
            } else if (remaining > 0) {
                state = "homestretch";
            } else {
                state = "after";
            }
        }
        let otherStates = ["before", "during", "homestretch", "after"];
        otherStates.splice(otherStates.indexOf(state), 1);

        // inactive states
        for (o in otherStates) {
            let elements = document.getElementsByClassName(otherStates[o]);
            for (let i = 0; i < elements.length; i++) {
                elements[i].style.display = "none";
            }
        }

        // active states
        let elements = document.getElementsByClassName(state);
        for (let i = 0; i < elements.length; i++) {
            elements[i].style.display = "block";
        }
    }, 100);
</script>
#+END_SRC


* Versión DEMO del temporizador genérico

Temporizador con una duración de 2 minutos. Durante el último minuto se muestran las instrucciones correspondientes a los últimos momentos antes de finalizar el examen (es lo que se ve en los últimos 15 minutos de los demás exámenes).

#+name: DURACIÓN 2 Y TEXTO DE LA ESFERA
#+BEGIN_SRC html :noweb yes
<script>
    var duration = 2;
    var minutosfinales = 1;
    <<TEXTO DE LA ESFERA>>
</script>
#+END_SRC


#+BEGIN_SRC html :tangle docs/temporizadores/Temporizador-demo.html :noweb yes
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title>Temporizador</title>

    <<DURACIÓN 2 Y TEXTO DE LA ESFERA>>
      
    <<definición de estilo>>
  </head>
  <body>
    
    <<INFORMACIÓN GENERAL>>

    <<sección temporizador>>
    <<script funcionamiento>>
  </body>
</html>    
#+END_SRC


